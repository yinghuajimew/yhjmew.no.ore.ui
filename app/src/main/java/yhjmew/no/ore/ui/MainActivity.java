...
public class MainActivity extends Activity {
    ...
    private static final int SO_TYPE_WITH_PRELOADER = 0;     // 需要 libpreloader.so
private static final int SO_TYPE_STANDALONE = 1;          // 不需要依赖
private static final String PREF_SO = "SoConfig";

private int currentSoType = SO_TYPE_STANDALONE;           // 默认独立版
...
printLog(getString(R.string.current_so_type) + getSoDisplayName(currentSoType));  // ← 新增
    }
...
if (currentSoType == SO_TYPE_WITH_PRELOADER) {
    ...
    updateLogSafe(getString(R.string.inject_with_preloader_mode));
} else {
    soAssetName = "libForceCloseOreUI_standalone.so";
    updateLogSafe(getString(R.string.inject_standalone_mode));
}

// 注入时统一命名为 libForceCloseOreUI.so
...
private void alignTo4K(ZipOutputStream zos) throws Exception {
    // 获取当前 ZIP 流已写入的字节数
    // ZipOutputStream 没有公开 API，我们用反射获取
    try {
        ...
        // 如果反射失败，说明系统版本或实现不同，跳过对齐
        // Android 11+ 一般不需要手动对齐
    }
}
    ...
    SharedPreferences prefs = getSharedPreferences(PREF_SO, MODE_PRIVATE);
...
private String getSoDisplayName(int type) {
    if (type == SO_TYPE_WITH_PRELOADER) {
        return getString(R.string.so_with_preloader);
    } else if (type == SO_TYPE_STANDALONE) {
        return getString(R.string.so_standalone);
    }
    return getString(R.string.unknown_so_type);
}
...
private void showSoConfigDialog() {
    ...
    rbStandalone.setId(SO_TYPE_STANDALONE);
    ...
    rbWithPreloader.setId(SO_TYPE_WITH_PRELOADER);
    radioGroup.addView(rbWithPreloader);

    radioGroup.check(currentSoType);
    container.addView(radioGroup);
    ...
    new android.app.AlertDialog.Builder(this)
            ...
            .setPositiveButton(getString(R.string.sure), new android.content.DialogInterface.OnClickListener() {
                ...
                public void onClick(android.content.DialogInterface dialog, int which) {
                    int selectedId = radioGroup.getCheckedRadioButtonId();
                    currentSoType = selectedId;
                    saveSoConfig();
                    printLog(getString(R.string.switched_to) + getSoDisplayName(currentSoType));
                }
            })
            .setNegativeButton(getString(R.string.cancel), null)
            .show();
