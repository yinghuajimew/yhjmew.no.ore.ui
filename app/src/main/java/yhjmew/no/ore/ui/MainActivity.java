...
public class MainActivity extends Activity {
    ...
    private void showSignConfigDialog() {
        ...
        String[] signNames = new String[]{getString(R.string.default_mt_signature), getString(R.string.zihao_il_signature), getString(R.string.custom_signaturebks_signature_files_only)};
    ...
    private void injectFileIntoApk(File sourceFile, File destFile, String oldPkg, String targetPkg, boolean needClone)
        ...
        try {
            ...
            if (needClone) {
                updateLogSafe(getString(R.string.executing_so_path_modification));
                File cacheDir = getExternalCacheDir();
                if (cacheDir == null) cacheDir = getCacheDir();
                ...
                try {
                    ...
                    updateLogSafe(getString(R.string.successfully_modified_and_injected_libforcecloseor));
                } catch (Exception e) {
                    updateLogSafe(getString(R.string.failed_to_modify_so) + getFullStackTrace(e));
    ...
    private void replaceStringInSoFile(File soFile, String oldStr, String newStr, byte padChar)
            throws Exception {
        if (!soFile.exists()) {
            throw new Exception(getString(R.string.so_file_does_not_exist) + soFile.getAbsolutePath());
        }

        if (soFile.length() == 0) {
            throw new Exception(getString(R.string.so_file_size_is_0_bytes) + soFile.getAbsolutePath());
        }
        ...
        if (newBytes.length > oldBytes.length) {
            throw new Exception(getString(R.string.so_new_path_length) + newBytes.length + getString(R.string.bytes_exceeds_the_old_path_length)
                    + oldBytes.length + getString(R.string.bytes_and_cannot_be_replaced_n)
                    + getString(R.string.old_path) + oldStr + getString(R.string.nnew_path) + newStr);
        }
        ...
        if (isModified) {
            ...
            fos.write(fileBytes);
            fos.close();
            updateLogSafe(getString(R.string.so_found_in_so) + matchCount + getString(R.string.match_and_replace));
        } else {
            throw new Exception(getString(R.string.so_target_path_pattern_not_found_in_file) + oldStr + "\n"
                    + getString(R.string.file_size) + soFile.length() + getString(R.string.bytesn)
                    + getString(R.string.possible_reasons_this_path_is_not_hardcoded_in_the));
        }
    }
    ...
    private void injectLocalFileToZip(ZipOutputStream zos, File localFile, String zipPath)
            throws Exception {
